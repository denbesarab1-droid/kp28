...
async function inviteUserToRoom() {
  ...
  try {
    const res = await fetch(`https://matrix.org/_matrix/client/r0/rooms/${this.roomId}/invite`, {
      ...
      body: JSON.stringify({ user_id: invited })
    });
    const data = await res.json();
    if (data.errcode) {
...
async function joinRoom() {
  ...
  try {
    const res = await fetch(`https://matrix.org/_matrix/client/r0/join/${encodeURIComponent(target)}`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${this.accessToken}` }
    });
    const data = await res.json();
    if (data.room_id) {
      this.roomId = data.room_id;          // використовуємо ідентифікатор з відповіді
      this.joinRoomId = '';
      this.messages = [];
      this.lastSyncToken = '';
      if (typeof this.fetchRoomsWithNames === 'function') {
